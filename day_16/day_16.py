    ...
    while i < len(bin_signal):
        ...
        if is_literal:
            ...
            if is_bits_length:
                ...
                unpack(bin_signal[t_i+16:t_i+16+length_int], versions)
                i = t_i+16+length_int
                ...
                for p in range(1, length_int+1):
                    # unpack(bin_signal[start + 11 * p:start + 11 * p + 11], versions)
                    unpack(bin_signal[start + 11 * p:], versions)  # TODO: how to come out of it? return from the "literal number" case as it's always a leaf?
